...
<style type="text/css">
	...
	#first{
		...
		background-color: rgba(214, 244, 250, 0.9);
	}
</style>
			...
			$.ajax({
				...
				success : function(rep){
					...
					rep.forEach(function(data){
						...
						writeDate += date.getMonth()+1+"월"
						writeDate += date.getDate()+"일"
						writeDate += date.getHours()+"시"
...
<body onload="replyData()">
	...
	<tr>
		<th width="100">글 번호</th> <th width="200"> ${personalData.writeNo }</th>
		<th width="100">작성자</th> <th width="200"> ${personalData.id }</th>
	</tr>
	<tr>
	...
	<th>내 용</th> <td>${personalData.content }</td>
		<td colspan="2">
			<c:if test="${personalData.imageFileName == 'nan' }">이미지가 없다 </c:if>
	...
	<tr>
		...
		<div id="reply"></div>
			...
			<c:if test="${loginUser == personalData.id }">
				...
				<input type="button" onclick="history.back()" value="리스트로 돌아가기">
		</td>
	</tr>
	</table>
